...
# 15.6.2020 "-b" option (Both genders) This code is not ready!!!
############################################################################
...
if (scalar @bres == 1){
	$bopt=2; #sex column id
	print "Using both genders..";
}
...
# Europe standard
my @wstd=(0.08,0.07,0.07,0.07,0.07,0.07,0.07,0.07,0.07,0.07,0.07,0.06,0.05,0.04,0.03,0.02,0.01,0.01);
...
# optimoidaan popdatan hakuja rajaamalla data vuosiin
	...
	my @gpcd=group_by(@pcdata,\@cols,$bopt);
	my @cancerpd=();
...
if($runtype==2)
{
	...
	#	my $ccode=
		...
		if ($found==0)
		{
			...
			print join(",",@outrow) . "\n";
			if (!defined $outrow[2]){
				print STDERR "sex=".$sex ."file:". $ARGV[0];
			} 	
			#}
			#else {
			#	print "Error: in row 419: " .$ARGV[0] . "- ";
			#}
		}
	}
}
...
sub group_by
{
	my $sexcol=@{pop(@_)}; # -1 default functioning -> if both sexes -> sex column to ignore (should be 2 based on the data)
	my @ocols=@{pop(@_)};
	my @nd=();
	my @data=();
	my @cols=();
...
sub print_usage
{
	...
	print "-b : 'sex=2 ~ both genders - when population data genders [0,1]'";
